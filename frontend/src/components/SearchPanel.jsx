    ...
    <div className="space-y-8 flex flex-col h-full">
      ...
      {!searched && !loading && (
        <div className="flex-1 flex flex-col items-center justify-center text-center p-12 glass-card">
          ...
          <h3 className="text-slate-300 font-medium mb-2 text-lg">Deep Semantic Search</h3>
          <p className="text-slate-500 text-sm max-w-md">
            Find relevant chunks of text across your entire vector database. Searches understand meaning, not just exact keywords.
          </p>
        </div>
      )}
      ...
      {loading && (
        <div className="flex-1 flex flex-col items-center justify-center glass-card">
          ...
          <span className="mt-4 text-sm text-slate-400 font-medium tracking-wide">Searching Vector Space...</span>
        </div>
      )}

      {searched && !loading && results.length === 0 && (
        <div className="flex-1 flex items-center justify-center glass-card p-8">
          <div className="text-slate-500 italic">No relevant chunks found. Try another query.</div>
        </div>
      )}
      ...
      {searched && !loading && results.length > 0 && (
        <div className="flex-1 overflow-y-auto pr-2 pb-8">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xs uppercase tracking-widest text-slate-400">Top Matches</h3>
          </div>
                ...
                <div className="p-6 pl-8">
                  ...
                  <div className="mt-auto">
                    <div className="flex justify-between items-center mb-1.5">
                      <span className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Match Score</span>
